...
Для удобства управления мультимедийным контентом в системе реализована административная web-панель.

Админка позволяет работать с файлами напрямую через браузер без необходимости ручного доступа к папкам проекта и файловой системе сервера.

После запуска backend-сервиса панель управления доступна по адресу:

http://127.0.0.1:8000/admin

Если подключение выполняется с другого компьютера внутри локальной сети, используется адрес:

http://IP_сервера:8000/admin

Через административную панель доступны следующие возможности:

1. Загрузка одного или нескольких файлов одновременно;
2. Удаление файлов;
3. Просмотр списка текущего мультимедийного контента;
4. Уведомления об успешной загрузке и удалении файлов;
5. Удалённое управление контентом без прямого доступа к серверу.

Все файлы, загруженные через админку, автоматически сохраняются в папку `tv_content/common`, после чего становятся доступны для формирования общего M3U-плейлиста.

Это позволяет значительно упростить сопровождение системы и отказаться от ручного копирования файлов в рабочие директории проекта.